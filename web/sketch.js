...
let isLegalCell = cell => {
  if (!isInside(cell)) return false;
    ...
    console.log(cell, grid[cell.y][cell.x])
    return (
...
mouseClicked = () => {
  ...
  if (isLegalCell(cell) && !isExploding() && turn == 0 && getUnstableCells().length == 0) {
    let pos = gridToPixel(cell);
    grid[cell.y][cell.x].push(createRobot(pos.x, pos.y, turn));
    switchTurn();
    checkUnstablesAndExplode();
...
draw = function() {
  ...
  if (turn == 1 && !isExploding() && getUnstableCells().length == 0 && !waiting) {
    waiting = true;
    fetchJSON("/api/move/", {
      method: "POST",
      body: JSON.stringify(boardToArray(grid)),
      headers: {
...
fetchJSON = (url, options, callback) => {
